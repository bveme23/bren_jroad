    ...
    <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          {subtitle ? <p className="mt-4 max-w-2xl text-base text-muted-foreground md:text-lg leading-relaxed">{subtitle}</p> : null}
        </div>
      </div>
        ...
        {items.map((item) => (
          <div key={item.name} className="flex h-full flex-col gap-4 rounded-2xl border border-subtle bg-card p-10 shadow-japanese hover-lift hover-glow">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-muted-foreground">{item.location}</p>
            </div>
